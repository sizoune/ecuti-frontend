...
export function useCutiList(filters: CutiFilters = {}) {
	return useQuery({
		...
		queryFn: async () => {
			...
			const { data } = await api.get<PaginatedResponse<Cuti>>(
				`/cuti?${params}`,
			);
			return data;
		},
...
export function useCutiStatistics(
	skpdId?: number,
	tahun?: number,
	enabled = true,
) {
	return useQuery({
		...
		queryFn: async () => {
			...
			const { data } = await api.get<CutiStatisticsResponse>(
				`/cuti/statistics?${params}`,
			);
			...
			return {
				total_pengajuan: Array.from(statusMap.values()).reduce(
					(a, b) => a + b,
					0,
				),
...
export function useCreateCuti() {
	...
	return useMutation({
		mutationFn: async (payload: CreateCutiRequest) => {
			const { data } = await api.post<{ usulcuti_id: number }>(
				"/cuti",
				payload,
			);
			return data;
...
export function useUploadFilePengantar(id: number) {
	const queryClient = useQueryClient();
	return useMutation({
		mutationFn: async (file: File) => {
			const formData = new FormData();
			formData.append("file", file);
			const { data } = await api.post(`/cuti/${id}/file-pengantar`, formData, {
		...
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ["cuti"] });
			queryClient.invalidateQueries({ queryKey: ["cuti", id] });
			queryClient.invalidateQueries({ queryKey: ["cuti-file-pengantar", id] });
		},
	});
}

export function useUploadFileSK(id: number) {
	const queryClient = useQueryClient();
	return useMutation({
		mutationFn: async (file: File) => {
			const formData = new FormData();
			formData.append("file", file);
			const { data } = await api.post(`/cuti/${id}/file-sk`, formData, {
				headers: { "Content-Type": "multipart/form-data" },
			});
			return data;
		},
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ["cuti-file-sk", id] });
		},
	});
}

export function useFilePengantarUrl(id: number, enabled = true) {
	return useQuery({
		queryKey: ["cuti-file-pengantar", id],
		queryFn: async () => {
			const { data } = await api.get<{ url: string }>(
				`/cuti/${id}/file-pengantar`,
			);
			return data;
		},
		enabled: !!id && enabled,
...
export function useFileSKUrl(id: number, enabled = true) {
	return useQuery({
		queryKey: ["cuti-file-sk", id],
		queryFn: async () => {
			const { data } = await api.get<{ url: string }>(`/cuti/${id}/file-sk`);
			return data;
		},
		enabled: !!id && enabled,
		retry: false,
...
export function useCreateBalance() {
	const queryClient = useQueryClient();
	return useMutation({
		mutationFn: async (payload: CreateBalanceRequest) => {
			const { data } = await api.post<CutiBalanceEntry>(
				"/cuti/balance",
				payload,
			);
			return data;
...
export function useUpdateBalance(id: number) {
	const queryClient = useQueryClient();
	return useMutation({
		mutationFn: async (payload: UpdateBalanceRequest) => {
			const { data } = await api.patch<CutiBalanceEntry>(
				`/cuti/balance/${id}`,
				payload,
			);
			return data;
...
export function useCutiBalanceEntries(pegawaiId?: number, tahun?: number) {
	const path = pegawaiId ? `/cuti/balance/${pegawaiId}` : "/cuti/balance";
	return useQuery({
		queryKey: ["cuti-balance-entries", pegawaiId, tahun],
		queryFn: async () => {
			const params = tahun ? `?tahun=${tahun}` : "";
			const { data } = await api.get<CutiBalanceEntry[]>(`${path}${params}`);
			return data;
