...
import { Loader2, IdCard, User, Building2, Briefcase, Shield, Phone, Mail, Award, Users } from 'lucide-react'
...
function getRoleColor(role: string | undefined) {
  switch (role) {
    ...
    case 'Admin SKPD': return 'default'
    case 'Admin Uker': return 'secondary'
        ...
        <CardContent className="pt-6 pb-6">
          <div className="flex items-center gap-5">
            ...
            <div className="min-w-0">
              ...
              <p className="text-sm text-muted-foreground mt-0.5 font-mono">{profile?.pegawai_nip ?? user?.pegawai_nip ?? '-'}</p>
              <div className="mt-2 flex flex-wrap gap-2">
                <Badge variant={getRoleColor(user?.role) as 'default' | 'secondary' | 'outline' | 'destructive'}>
                  {user?.role ?? 'Pegawai'}
                </Badge>
      ...
      <Card>
        ...
        <CardContent className="p-0">
          <div className="divide-y">
            <InfoRow icon={<IdCard className="h-3.5 w-3.5" />} label="NIP" value={profile?.pegawai_nip ?? user?.pegawai_nip ?? '-'} />
            <InfoRow icon={<User className="h-3.5 w-3.5" />} label="Nama" value={displayName} />
            <InfoRow icon={<Building2 className="h-3.5 w-3.5" />} label="SKPD" value={profile?.skpd_nama ?? '-'} />
            <InfoRow icon={<Users className="h-3.5 w-3.5" />} label="Unit Kerja" value={profile?.subunit_nama ?? '-'} />
            <InfoRow icon={<Award className="h-3.5 w-3.5" />} label="Golongan" value={profile?.golongan_nama ?? '-'} />
            <InfoRow icon={<Briefcase className="h-3.5 w-3.5" />} label="Jabatan" value={profile?.jabatan_nama ?? '-'} />
            <InfoRow icon={<Shield className="h-3.5 w-3.5" />} label="Status" value={profile?.pegawai_kedudukanpns ?? '-'} />
            <InfoRow icon={<Shield className="h-3.5 w-3.5" />} label="Role Sistem" value={user?.role ?? '-'} isLast />
          </div>
        </CardContent>
      </Card>
...
function InfoRow({
  ...
  return (
    <div className={`flex items-start gap-3 px-6 py-3 ${isLast ? '' : ''} hover:bg-muted/30 transition-colors`}>
