...
function ManajemenUserPage() {
	...
	const { data, isLoading } = useUserRoleList({
		...
		limit: LIMIT,
		skpd_id:
			skpdFilter && skpdFilter !== "all" ? Number(skpdFilter) : undefined,
		role_id:
		...
		<div className="space-y-4">
			<div>
				...
				<p className="text-muted-foreground">
					Kelola peran pengguna sistem e-Cuti
				</p>
			</div>
								...
								<TableBody>
									{data?.data && data.data.length > 0 ? (
										data.data.map((item, idx) => (
											<TableRow
												key={`${item.pegawai_id}-${item.role_id}`}
											...
											>
												...
												<TableCell>
													<Badge
														variant="secondary"
														className={
															ROLE_BADGE_CLASS[item.role_id] ??
															"bg-gray-100 text-gray-700"
														}
													>
														{item.role_nama ??
															ROLE_LABELS[item.role_id] ??
															`Role ${item.role_id}`}
													</Badge>
												</TableCell>
												...
												<div className="flex flex-col items-center gap-3 text-muted-foreground">
													...
													<div>
														<p className="font-medium text-sm">
															Belum ada penugasan role
														</p>
														<p className="text-xs mt-1">
															Tambahkan role kepada pegawai menggunakan tombol
															di atas.
														</p>
													</div>
												</div>
			...
			<AlertDialogContent>
				<AlertDialogHeader>
					...
					<AlertDialogDescription>
						...
						dari{" "}
						<strong>
							{item.pegawai_nama ?? `Pegawai #${item.pegawai_id}`}
						</strong>
...
function AssignRoleDialog({
	...
	const showDropdown =
		showResults && debouncedSearch.length >= 3 && pegawaiResults.length > 0;
