...
import { CalendarDays, Download, FileUp, Plus, Trash2 } from "lucide-react";
...
import {
	...
	useCutiBersamaFileUrl,
	useCutiBersamaList,
	useDeleteCutiBersama,
	useUploadCutiBersamaFile,
		...
		<div className="space-y-4">
			...
			<Card>
				...
				<div className="flex flex-wrap items-center justify-between gap-3 border-b px-6 py-3">
					...
					{isSuperAdmin && <CreateCutiBersamaDialog tahun={tahun} />}
				</div>
									...
									{data?.data && data.data.length > 0 ? (
										data.data.map((item, idx) => (
											...
											>
												...
												<TableCell className="text-center">
													{item.cutibersama_file ? (
														<FileViewButton id={item.cutibersama_id} />
													) : (
												...
												{isSuperAdmin && (
													<TableCell className="text-center">
														<div className="flex items-center justify-center gap-1">
															<UploadFileDialog
																id={item.cutibersama_id}
																nama={item.cutibersama_nama}
															/>
															<DeleteCutiBersamaButton
																id={item.cutibersama_id}
																nama={item.cutibersama_nama}
															/>
														</div>
												...
												<div className="flex flex-col items-center gap-3 text-muted-foreground">
													...
													<div>
														...
														<p className="text-xs mt-1">
															Belum ada data cuti bersama untuk tahun {tahun}.
														</p>
													</div>
												</div>
											</TableCell>
...
function CreateCutiBersamaDialog({ tahun }: { tahun: number }) {
	...
	function handleSubmit(e: React.FormEvent) {
		...
		mutate(
			...
			{
				onSuccess: (result) => {
					if (pendingFile && result.cutibersama_id) {
						// Upload file after record is created
						const formData = new FormData();
						formData.append("file", pendingFile);
						import("#/lib/api").then(({ default: api }) => {
							api
								.post(`/cuti-bersama/${result.cutibersama_id}/file`, formData, {
									headers: { "Content-Type": "multipart/form-data" },
								})
								.then(() => {
									toast.success("Cuti bersama dan file berhasil ditambahkan");
								})
								.catch(() => {
									toast.error(
										"Cuti bersama ditambahkan, tapi gagal mengunggah file",
									);
								});
						});
					} else {
			...
			<DialogContent className="sm:max-w-md">
				...
				<form onSubmit={handleSubmit}>
					<div className="space-y-4 py-2">
						...
						<div className="space-y-1.5">
							<Label htmlFor="cb-file">
								File Pendukung (opsional, PDF, maks. 2MB)
							</Label>
							<Input
								id="cb-file"
								type="file"
...
function FileViewButton({ id }: { id: number }) {
	const [enabled, setEnabled] = useState(false);
	const { data, isFetching } = useCutiBersamaFileUrl(id, enabled);
...
function UploadFileDialog({ id, nama }: { id: number; nama: string }) {
	const [open, setOpen] = useState(false);
	const [file, setFile] = useState<File | null>(null);
	const { mutate, isPending } = useUploadCutiBersamaFile(id);
		...
		>
			...
			<DialogContent className="sm:max-w-sm">
				<DialogHeader>
					...
					<DialogDescription>
						Unggah file pendukung untuk <strong>{nama}</strong>.
					</DialogDescription>
				</DialogHeader>
				<form onSubmit={handleSubmit}>
					<div className="space-y-3 py-2">
						<div className="space-y-1.5">
							<Label htmlFor={`upload-file-${id}`}>File (PDF, maks. 2MB)</Label>
							<Input
								id={`upload-file-${id}`}
								type="file"
								accept=".pdf"
...
function DeleteCutiBersamaButton({ id, nama }: { id: number; nama: string }) {
